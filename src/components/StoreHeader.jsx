...
export default function StoreHeader() {
    ...
    return (
        <>
            <header className="bg-white border-b border-gray-100 sticky top-0 z-40 py-4">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between">
                    <div className="md:hidden">
                        <button onClick={() => setIsMenuOpen(true)} className="p-2 rounded-md text-[#1E3A8A]">
                            <Menu size={28} />
                        </button>
                    </div>
                    ...
                    <nav className="hidden md:flex items-center gap-10">
                        ...
                        <NavLink to="/about" className={({isActive}) => `text-[11px] font-black uppercase tracking-widest ${isActive ? 'text-[#1E3A8A]' : 'text-gray-400'} hover:text-[#1E3A8A]`}>About</NavLink>
                        <NavLink to="/orders" className={({isActive}) => `text-[11px] font-black uppercase tracking-widest ${isActive ? 'text-[#1E3A8A]' : 'text-gray-400'} hover:text-[#1E3A8A]`}>Orders</NavLink>
                    </nav>
            ...
            {/* MOBILE MENU */}
            <div className={`fixed inset-0 bg-black/60 z-50 transition-opacity duration-300 md:hidden ${isMenuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} onClick={() => setIsMenuOpen(false)}></div>
            <div className={`fixed top-0 left-0 h-full w-4/5 max-w-xs bg-white z-50 shadow-2xl transition-transform duration-300 ease-in-out md:hidden ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                <div className="p-6 flex flex-col h-full">
                    <div className="flex items-center justify-between pb-4 border-b border-gray-200">
                        <Link to="/" onClick={handleLinkClick} className="flex items-center gap-3">
                         ...
                         <button onClick={() => setIsMenuOpen(false)} className="p-2 -mr-2 rounded-md text-gray-500">
                             <X size={24} />
                         </button>
                    </div>
                    ...
                    <nav className="flex-grow mt-8">
                        ...
                        <Link to="/contact" onClick={handleLinkClick} className="block py-4 text-lg font-black uppercase tracking-widest text-gray-600 border-b border-gray-100">Contact</Link>
                        <Link to="/about" onClick={handleLinkClick} className="block py-4 text-lg font-black uppercase tracking-widest text-gray-600 border-b border-gray-100">About Us</Link>
                        <Link to="/orders" onClick={handleLinkClick} className="block py-4 text-lg font-black uppercase tracking-widest text-gray-600 border-b border-gray-100">Orders</Link>
                    </nav>
