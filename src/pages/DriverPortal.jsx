...
import { Truck, Store, Package, Loader2, CheckCircle2, ArrowLeft, MapPin, LogOut } from 'lucide-react';
            ...
            <div className="bg-[#002147] text-white p-8 rounded-b-[45px] sticky top-0 z-10 shadow-xl">
                <div className="flex justify-between items-center mb-8">
                    <div className="flex items-center gap-4">
                        ...
                        <h1 className="text-xl font-black uppercase italic tracking-tighter">Driver Portal</h1>
                    </div>
                    <button onClick={signOut}><LogOut size={20} className="text-blue-300" /></button>
                </div>
                <div className="flex bg-[#00142b] p-2 rounded-[30px] gap-2">
                    <button onClick={() => setView('pickup')} className={`flex-1 py-4 rounded-[25px] font-black uppercase text-[10px] tracking-widest ${view === 'pickup' ? 'bg-white text-[#002147]' : 'text-blue-300'}`}>SHOP ({pickupList.length})</button>
                    <button onClick={() => setView('delivery')} className={`flex-1 py-4 rounded-[25px] font-black uppercase text-[10px] tracking-widest ${view === 'delivery' ? 'bg-white text-[#002147]' : 'text-blue-300'}`}>TRUCK ({deliveryList.length})</button>
                    <button onClick={() => setView('history')} className={`flex-1 py-4 rounded-[25px] font-black uppercase text-[10px] tracking-widest ${view === 'history' ? 'bg-white text-[#002147]' : 'text-blue-300'}`}>HISTORY</button>
                </div>
            </div>
                ...
                {(view === 'history' ? weeklyHistory : view === 'pickup' ? pickupList : deliveryList).map(order => (
                    <div key={order.id} className="bg-white p-8 rounded-[40px] border border-gray-100 shadow-sm">
                        <div className="flex justify-between mb-2">
                            ...
                            <p className="font-black text-[#002147]">R {order.total_amount}</p>
                        </div>
                        ...
                        <button onClick={() => { setOpenOrderId(openOrderId === order.id ? null : order.id); loadOrderItems(order.id); }} className="w-full mt-4 text-[10px] font-black text-gray-300 uppercase tracking-widest">
                            {openOrderId === order.id ? 'Hide Details' : 'View Items'}
                        </button>
                        {openOrderId === order.id && (
                            <div className="mt-4 pt-4 border-t border-gray-50 space-y-2">
                                {orderItems[order.id]?.map((item, i) => (
                                    <p key={i} className="text-xs font-bold text-gray-600">{item.quantity}x {item.product_id?.name}</p>
                                ))}
                            </div>
                        )}
                    </div>
